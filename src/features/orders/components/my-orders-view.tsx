...
const statusConfig: Record<OrderStatus, { label: string; className: string }> = {
  ...
  return_requested: { label: "Chờ duyệt hoàn trả", className: "bg-orange-50 text-orange-700" },
  returned: { label: "Đã hoàn trả", className: "bg-emerald-50 text-emerald-700" },
  return_rejected: { label: "Từ chối hoàn trả", className: "bg-red-50 text-red-700" },
};
...
export function MyOrdersView() {
  ...
  const [action, setAction] = useState<{ type: "cancel" | "return"; order: OrderSummary }>();
  ...
  async function submitAction(reason: string, files: File[]) {
    ...
    try {
      ...
      toast.success(action.type === "cancel" ? "Đã hủy đơn và hoàn lại tồn kho" : "Đã gửi yêu cầu hoàn trả");
      setAction(undefined);
      if (detail?.id === updated.id) setDetail(updated);
      await loadOrders();
    } catch (error) {
      toast.error(error instanceof ApiError ? error.message : "Không thể xử lý yêu cầu.");
    } finally {
    ...
    <div>
      ...
      {loading ? <div className="space-y-4">{Array.from({ length: 4 }).map((_, index) => <div key={index} className="h-44 animate-pulse rounded-2xl border bg-white" />)}</div> : orders.length === 0 ? <div className="rounded-2xl border bg-white p-16 text-center"><PackageSearch className="mx-auto size-14 text-zinc-300" /><h2 className="mt-4 text-xl font-bold">Chưa có đơn hàng</h2><p className="mt-2 text-sm text-muted-foreground">Không tìm thấy đơn hàng phù hợp với trạng thái đã chọn.</p><Button className="mt-5 bg-[#ff5a1f] text-white" onClick={() => router.push("/")}>Tiếp tục mua sắm</Button></div> : <div className="space-y-4">
        {orders.map((order) => <article key={order.id} className="overflow-hidden rounded-2xl border bg-white">
          ...
          <div className="grid gap-5 p-5 md:grid-cols-[1fr_auto] md:items-center"><div className="grid gap-3 text-sm sm:grid-cols-3"><div><p className="text-xs text-muted-foreground">Người nhận</p><p className="mt-1 font-medium">{order.recipientName}</p></div><div><p className="text-xs text-muted-foreground">Số sản phẩm</p><p className="mt-1 font-medium">{order.itemCount} sản phẩm</p></div><div><p className="text-xs text-muted-foreground">Tổng thanh toán</p><p className="mt-1 text-lg font-bold text-[#ff5a1f]">{money(order.totalAmount)}</p></div>{order.rejectionReason && <div className="rounded-lg bg-red-50 p-3 text-red-700 sm:col-span-3"><strong>Lý do từ chối:</strong> {order.rejectionReason}</div>}{order.cancellationReason && <div className="rounded-lg bg-zinc-50 p-3 text-zinc-700 sm:col-span-3"><strong>Lý do hủy:</strong> {order.cancellationReason}</div>}{order.returnReviewReason && <div className="rounded-lg bg-orange-50 p-3 text-orange-800 sm:col-span-3"><strong>Phản hồi hoàn trả:</strong> {order.returnReviewReason}</div>}</div><div className="flex flex-wrap justify-end gap-2"><Button variant="outline" onClick={() => void openDetail(order.id)}><Eye /> Xem chi tiết</Button>{(order.status === "pending" || order.status === "confirmed") && <Button variant="outline" className="text-red-600" onClick={() => setAction({ type: "cancel", order })}><Ban /> Hủy đơn</Button>}{canRequestReturn(order) && <Button className="bg-[#ff5a1f] text-white hover:bg-[#e94b13]" onClick={() => setAction({ type: "return", order })}><RotateCcw /> Yêu cầu trả hàng</Button>}</div></div>
        </article>)}
      </div>}
      ...
      {action && <OrderActionDialog type={action.type} order={action.order} pending={actionPending} onClose={() => { if (!actionPending) setAction(undefined); }} onSubmit={submitAction} />}
    </div>
  );
}
...
function canRequestReturn(order: OrderSummary): boolean {
  ...
  return Date.now() <= new Date(order.confirmedAt).getTime() + 7 * 24 * 60 * 60 * 1000;
}
...
function OrderDetailDialog({ order, loading, onClose }: { order?: Order; loading: boolean; onClose: () => void }) {
  return <div className="fixed inset-0 z-50 grid place-items-center bg-black/45 p-4 backdrop-blur-sm" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><div role="dialog" aria-modal="true" className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-2xl bg-white p-5 shadow-2xl sm:p-7">
    ...
    {loading || !order ? <div className="grid min-h-64 place-items-center"><LoaderCircle className="size-8 animate-spin text-[#ff5a1f]" /></div> : <div className="mt-5 space-y-5">
      ...
      {order.cancellationReason && <div className="rounded-xl border bg-zinc-50 p-4 text-sm"><strong>Lý do hủy:</strong> {order.cancellationReason}</div>}
      {order.returnReason && <div className="rounded-xl border border-orange-200 bg-orange-50 p-4 text-sm text-orange-800"><strong>Lý do yêu cầu hoàn trả:</strong> {order.returnReason}</div>}
      {order.returnEvidence.length > 0 && <EvidenceGallery evidence={order.returnEvidence} />}
      {order.returnReviewReason && <div className="rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-800"><strong>Phản hồi của cửa hàng:</strong> {order.returnReviewReason}</div>}
...
function OrderActionDialog({ type, order, pending, onClose, onSubmit }: { type: "cancel" | "return"; order: OrderSummary; pending: boolean; onClose: () => void; onSubmit: (reason: string, files: File[]) => Promise<void> }) {
  const [reason, setReason] = useState("");
  const [files, setFiles] = useState<File[]>([]);
  const [error, setError] = useState("");
  const [queued, setQueued] = useState(false);
  const busy = pending || queued;
  const submitAction = useDebouncedCallback(async (value: string, selectedFiles: File[]) => {
    try {
  ...
  function selectFiles(selected: FileList | null) {
    ...
    const invalid = next.find((file) => {
      ...
      return (!video && !file.type.startsWith("image/")) || file.size > (video ? 50 * 1024 * 1024 : 5 * 1024 * 1024);
    });
    if (invalid) {
  ...
  return <div className="fixed inset-0 z-[60] grid place-items-center bg-black/45 p-4 backdrop-blur-sm" onMouseDown={(event) => { if (event.target === event.currentTarget && !busy) onClose(); }}><div role="dialog" aria-modal="true" className="max-h-[90vh] w-full max-w-xl overflow-y-auto rounded-2xl bg-white p-6 shadow-2xl"><div className="flex items-start justify-between"><div><p className="text-xs font-bold uppercase tracking-wider text-[#ff5a1f]">{order.orderCode}</p><h2 className="mt-1 text-xl font-bold">{type === "cancel" ? "Hủy đơn hàng" : "Yêu cầu hoàn trả hàng"}</h2><p className="mt-1 text-sm text-muted-foreground">{type === "cancel" ? "Tồn kho sẽ được hoàn lại ngay sau khi hủy thành công." : "Chỉ áp dụng trong 7 ngày kể từ khi đơn được xác nhận."}</p></div><Button size="icon-sm" variant="ghost" disabled={busy} onClick={onClose}><X /></Button></div><div className="mt-5 space-y-4"><label className="block space-y-2 text-sm font-medium">Lý do<textarea value={reason} onChange={(event) => { setReason(event.target.value); setError(""); }} maxLength={1000} disabled={busy} className="min-h-28 w-full resize-y rounded-xl border p-3 outline-none focus:border-[#ff5a1f] focus:ring-2 focus:ring-orange-100" placeholder={type === "cancel" ? "Nhập lý do hủy đơn..." : "Mô tả tình trạng sản phẩm cần hoàn trả..."} /></label>{type === "return" && <div><label className="flex cursor-pointer items-center justify-center gap-2 rounded-xl border border-dashed p-4 text-sm font-semibold text-[#ff5a1f] hover:bg-orange-50"><ImagePlus className="size-5" /> Thêm hình ảnh hoặc video<input type="file" multiple accept="image/*,video/*" className="hidden" disabled={busy || files.length >= 6} onChange={(event) => { selectFiles(event.target.files); event.currentTarget.value = ""; }} /></label><p className="mt-1 text-xs text-muted-foreground">Tối đa 6 tệp · ảnh 5 MB · video 50 MB.</p>{files.length > 0 && <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3">{files.map((file, index) => <SelectedEvidence key={`${file.name}-${file.lastModified}-${index}`} file={file} onRemove={() => setFiles((current) => current.filter((_, itemIndex) => itemIndex !== index))} />)}</div>}</div>}{error && <p className="text-sm text-red-600">{error}</p>}<div className="flex justify-end gap-2"><Button variant="outline" disabled={busy} onClick={onClose}>Đóng</Button><Button disabled={busy} className={type === "cancel" ? "bg-red-600 text-white hover:bg-red-700" : "bg-[#ff5a1f] text-white hover:bg-[#e94b13]"} onClick={submit}>{busy && <LoaderCircle className="animate-spin" />}{type === "cancel" ? "Xác nhận hủy" : "Gửi yêu cầu"}</Button></div></div></div></div>;
}

function SelectedEvidence({ file, onRemove }: { file: File; onRemove: () => void }) {
  const [url] = useState(() => URL.createObjectURL(file));
  useEffect(() => () => URL.revokeObjectURL(url), [url]);
  return <div className="relative overflow-hidden rounded-xl border bg-zinc-50">{file.type.startsWith("video/") ? <video src={url} className="h-28 w-full object-cover" muted /> : <img src={url} alt={file.name} className="h-28 w-full object-cover" />}<button type="button" className="absolute right-1 top-1 grid size-7 cursor-pointer place-items-center rounded-full bg-black/65 text-white" onClick={onRemove}><Trash2 className="size-3.5" /></button><p className="truncate px-2 py-1.5 text-[10px]">{file.name}</p></div>;
}

function EvidenceGallery({ evidence }: { evidence: Order["returnEvidence"] }) {
  return <div><p className="mb-2 text-sm font-semibold">Minh chứng hoàn trả</p><div className="grid grid-cols-2 gap-3 sm:grid-cols-3">{evidence.map((asset) => <a key={asset.publicId} href={asset.url} target="_blank" rel="noreferrer" className="overflow-hidden rounded-xl border bg-zinc-50">{asset.resourceType === "video" ? <video src={asset.url} controls className="h-32 w-full object-cover" /> : <img src={asset.url} alt="Minh chứng sản phẩm lỗi" className="h-32 w-full object-cover" />}</a>)}</div></div>;
}
