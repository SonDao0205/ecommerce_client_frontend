...
const isVideo = (url: string) =>
  /\/video\/upload\/|\.(mp4|webm|mov|m4v)(\?|$)/i.test(url);
...
export function ProductDetailView({
  ...
  >(
    () =>
      product.variants?.[0]?.children?.find((item) => item.stock > 0) ??
      product.variants?.[0]?.children?.[0],
  );
  ...
  function chooseParent(parent: ProductVariant) {
    ...
    setSelectedVariant(
      parent.children?.find((item) => item.stock > 0) ?? parent.children?.[0],
    );
    setQuantity(1);
  }
    ...
    <div className="min-h-screen bg-[#f6f6f6] text-[#222]">
      ...
      <main className="mx-auto max-w-7xl px-4 py-5 lg:px-6">
        ...
        <section className="grid gap-8 rounded-2xl border bg-white p-4 md:p-6 lg:grid-cols-2">
          <div className="grid gap-3 sm:grid-cols-[76px_1fr]">
            <div className="order-2 flex gap-2 overflow-x-auto sm:order-1 sm:flex-col">
              {media.map((url, index) => (
                <button
                  ...
                  className={cn(
                    "relative size-[72px] shrink-0 overflow-hidden rounded-lg border bg-zinc-50",
                    activeMedia === url && "border-2 border-[#ff5a1f]",
                  )}
                >
                  {isVideo(url) ? (
                    <video
            ...
            <div className="relative order-1 aspect-square overflow-hidden rounded-xl border bg-zinc-50 sm:order-2">
              ...
              {discount > 0 && (
                <span className="absolute top-3 right-3 rounded-md bg-red-500 px-2 py-1 text-xs font-bold text-white">
                  -{discount}%
                </span>
              )}
            </div>
          ...
          <div className="py-1">
            ...
            <div className="mt-7 flex flex-wrap items-center gap-3">
              <strong className="text-3xl text-red-600">{money(price)}</strong>
            ...
            <p className="mt-2 text-xs text-muted-foreground">
              Giá đã bao gồm VAT · SKU: {selectedVariant?.sku ?? product.sku}
            </p>

            {(product.variants?.length ?? 0) > 0 && (
              <>
                <OptionGroup title={product.variants?.[0]?.name ?? "Biến thể"}>
                  {product.variants?.map((parent) => (
                    <button
                      key={parent.id}
                      type="button"
                    ...
                    >
                      ...
                      <small className="mt-1 block text-[11px] text-muted-foreground">
                        {parent.children?.length ?? 0} lựa chọn
                      </small>
                    </button>
                  ))}
                </OptionGroup>
                <OptionGroup
                  title={selectedParent?.children?.[0]?.name ?? "Phiên bản"}
                >
                  {selectedParent?.children?.map((variant) => (
                    <button
                      key={variant.id}
                      type="button"
                      ...
                      className={cn(
                        "relative min-w-28 rounded-lg border px-4 py-3 text-sm disabled:opacity-40",
                    ...
                    >
                      ...
                      <small className="mt-1 block text-[11px] text-muted-foreground">
                        {money(Number(variant.unitPrice))}
                      </small>
                      {selectedVariant?.id === variant.id && (
            ...
            <div className="mt-3 grid gap-3 sm:grid-cols-[120px_1fr_1fr]">
              <div className="grid h-12 grid-cols-3 overflow-hidden rounded-lg border">
                <button
                  type="button"
                ...
                <span className="grid place-items-center border-x text-sm font-semibold">
                  {quantity}
                </span>
                <button
                  type="button"
              ...
              <Button
                ...
                className="h-12 border-[#ff5a1f] text-[#ff5a1f]"
              >
                <ShoppingCart /> {isAdding ? "Đang thêm..." : "Thêm vào giỏ"}
              </Button>
              <Button
                disabled={stock <= 0}
                onClick={openBuyNow}
                className="h-12 bg-[#ff5a1f] text-white hover:bg-[#e94b13]"
              >
                Mua ngay
              </Button>
            </div>
            <div className="mt-6 grid gap-3 rounded-xl border p-4 sm:grid-cols-3">
              <Service
                icon={Truck}
        ...
        <section className="mt-5 rounded-2xl border bg-white p-6">
          <h2 className="mb-5 text-xl font-bold">Mô tả sản phẩm</h2>
          <div
            className="prose max-w-none text-sm leading-7 text-zinc-700 [&_h2]:mb-3 [&_h2]:text-xl [&_h2]:font-bold [&_li]:ml-5 [&_li]:list-disc"
        ...
        <section className="mt-5 rounded-2xl border bg-white p-5">
          <div className="mb-5 flex items-center justify-between">
            ...
            <Link href="/" className="text-sm text-[#ff5a1f]">
              Xem tất cả
            </Link>
          </div>
...
function OptionGroup({
  ...
  return (
    <div className="mt-6">
      <h3 className="mb-2 text-sm font-semibold">Chọn {title.toLowerCase()}</h3>
...
function Service({
  ...
  return (
    <div className="flex gap-2">
      <Icon className="size-5 shrink-0 text-[#ff5a1f]" />
      <div>
