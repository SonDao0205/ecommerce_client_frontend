  ...
  return (
    <article className="group overflow-hidden rounded-2xl border bg-white transition duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-black/5">
      <div className="relative aspect-square overflow-hidden bg-zinc-50">
        <DebouncedProductLink href={href} className="absolute inset-0 block">
      ...
      <div className="p-4">
        ...
        <DebouncedProductLink href={href} className="line-clamp-2 min-h-11 text-[15px] font-semibold leading-5 hover:text-[#ff5a1f]">{product.name}</DebouncedProductLink>
        ...
        <Button nativeButton={false} variant="outline" className="mt-4 w-full border-[#ff5a1f] text-[#ff5a1f] hover:bg-[#ff5a1f] hover:text-white" render={<DebouncedProductLink href={href} />}><ShoppingCart /> Xem sản phẩm</Button>
      </div>
    </article>
  );
}
