...
import { DebouncedProductLink } from "@/src/components/common/debounced-product-link";
...
export function CartPageView() {
  ...
  const [flushingQuantities, setFlushingQuantities] = useState(false);
  const quantities = useDebouncedCartQuantity(updateItem, (error) => {
  ...
  const displayedItems = items.map((item) => ({
    ...item,
    quantity: quantities.quantityFor(item),
  }));
  const displayedTotal = displayedItems.reduce(
    (sum, item) => sum + item.unitPrice * item.quantity,
    0,
  );
  const displayedQuantity = displayedItems.reduce(
  ...
  async function handleClear() {
    try {
      await quantities.flush();
      await clearCart();
      toast.success("Đã làm trống giỏ hàng");
    } catch (error) {
      toast.error(
        error instanceof ApiError ? error.message : "Không thể xóa giỏ hàng.",
      );
    }
  }
  ...
  async function openCheckout() {
    ...
    setFlushingQuantities(true);
    try {
      await quantities.flush();
      router.push("/checkout?mode=cart");
    } catch {
      // Hook đã hiển thị lỗi; không chuyển trang khi giỏ hàng chưa đồng bộ.
    } finally {
      setFlushingQuantities(false);
    }
  }
  ...
  if (items.length === 0) {
    return (
      <div className="rounded-2xl border bg-white p-16 text-center">
        <ShoppingBag className="mx-auto size-14 text-zinc-300" />
        <h1 className="mt-5 text-2xl font-bold">Giỏ hàng đang trống</h1>
        <p className="mt-2 text-sm text-muted-foreground">
          Hãy chọn sản phẩm bạn yêu thích để bắt đầu mua sắm.
        </p>
        <Button
          nativeButton={false}
          className="mt-6 bg-[#ff5a1f] text-white"
    ...
    <div className="grid gap-6 lg:grid-cols-[1fr_340px]">
        ...
        <div className="flex items-center justify-between border-b px-5 py-4">
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              {user ? "Đang lưu trong tài khoản" : "Đang lưu trên thiết bị này"}
            </p>
          </div>
          <Button
            variant="ghost"
        ...
        <div className="divide-y">
          {displayedItems.map((item) => (
            <CartItemRow
              key={item.id}
              item={item}
              busy={busyId === item.id || flushingQuantities}
              onUpdate={(quantity) => quantities.changeQuantity(item, quantity)}
              onRemove={() =>
                run(item, async () => {
                  await quantities.flush();
      ...
      <aside className="h-fit rounded-2xl border bg-white p-5 lg:sticky lg:top-32">
        ...
        <div className="flex items-end justify-between py-5">
          ...
          <strong className="text-2xl text-[#ff5a1f]">
            {money(displayedTotal)}
          </strong>
        </div>
        <Button
          disabled={flushingQuantities}
          onClick={() => void openCheckout()}
          className="h-11 w-full bg-[#ff5a1f] text-white hover:bg-[#e94b13]"
        >
          {flushingQuantities
        ...
        {!user && (
          <p className="mt-3 text-center text-xs text-muted-foreground">
            <Link href="/login" className="font-semibold text-[#ff5a1f]">
              Đăng nhập
            </Link>{" "}
            để lưu giỏ hàng vào tài khoản.
          </p>
        )}
      </aside>
    </div>
  ...
  return (
    <div className="grid gap-4 p-5 sm:grid-cols-[96px_1fr_auto] sm:items-center">
      ...
      <div>
        ...
        {item.variant && (
          <p className="mt-1 text-xs text-muted-foreground">
            {item.variant.name}: {item.variant.value} · SKU: {item.variant.sku}
          </p>
        )}
        <strong className="mt-3 block text-[#ff5a1f]">
